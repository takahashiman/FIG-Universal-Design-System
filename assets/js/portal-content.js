...
&lt;/body&gt;</code></pre>
  ...
  /* ════════ Core / Components ════════ */
  'core/components/fig-sense':   { template: 'component', title: 'FIG Sense（総覧）',  description: '角丸・アイソレーション・最小サイズの総合ショーケース。', preview: 'preview/components-fig-sense.html', spec: null,                                  a11y: 'すべての構成部品は WCAG AA + 日本語禁則を満たすトークンを経由します。' },
  'core/components/button':      { template: 'component', title: 'Button',             description: '主要CTA は Pill（--radius-cta）、Secondary は 16px。', preview: 'preview/components-buttons.html',    spec: 'components/button.spec.md',           a11y: '<code>aria-disabled</code> 併記、icon-only は <code>ariaLabel</code> 必須、loading 時は <code>aria-busy</code>。' },
  'core/components/card':        { template: 'component', title: 'Card',               description: '慈愛のやわらかさを 16px 角丸で表現。Hero は 28px。',    preview: 'preview/components-bus-card.html',   spec: 'components/card.spec.md',             a11y: 'interactive 時は <code>&lt;button&gt;</code>/<code>&lt;a&gt;</code> で実装。<code>&lt;div onClick&gt;</code> は禁止。' },
  'core/components/input':       { template: 'component', title: 'Input',              description: 'テキスト・検索・数値入力。IME 配慮済み。',                 preview: 'preview/components-inputs.html',     spec: 'components/input.spec.md',            a11y: 'label 必須、placeholder で代用しない。IME 入力中はフィルタ抑制。' },
  'core/components/modal':       { template: 'component', title: 'Modal',              description: '注意を画面全体から奪う、取消困難な操作の確認。',         preview: null,                                  spec: 'components/modal.spec.md',            a11y: '<code>role="dialog"</code> + フォーカストラップ + Escape ハンドラ。' },
  'core/components/status-pill': { template: 'component', title: 'Status Pill',         description: '運行状態を一目で伝えるアイコン + ラベル。',                preview: 'preview/components-status-pills.html', spec: 'components/status-pill.spec.md',      a11y: '色だけで意味を伝えない。<code>role="status"</code> + <code>aria-live</code>。' },
  'core/components/bottom-navigation': { template: 'component', title: 'Bottom Navigation', description: '画面下端固定のグローバルナビ。3–5タブ。',         preview: null,                                  spec: 'components/bottom-navigation.spec.md', a11y: '<code>&lt;nav aria-label&gt;</code> + 各アイテムに <code>aria-current</code>。' },
  'core/components/icon-bubble': { template: 'component', title: 'Icon Bubble',        description: '装飾的アイコンコンテナ。インタラクティブにしない。',     preview: null,                                  spec: 'components/icon-bubble.spec.md',      a11y: '既定で <code>aria-hidden="true"</code>。意味は親要素で集約表現。' },
  'core/components/list-item':   { template: 'component', title: 'List Item',          description: '垂直スタックの1行原子。Navigation/Selection/Action/Info。', preview: 'preview/components-list-rows.html', spec: 'components/list-item.spec.md',        a11y: '必ず <code>&lt;button&gt;</code> か <code>&lt;a&gt;</code> で実装。' },
  'core/components/tab':         { template: 'component', title: 'Tab',                description: '並列で排他的なビューを切り替える。3タブ以上は再検討。',  preview: 'preview/components-tabs.html',       spec: 'components/tab.spec.md',              a11y: '<code>role="tablist"</code> / <code>role="tab"</code> / <code>aria-selected</code> + ←→ キー。' },
  'core/components/toast':       { template: 'component', title: 'Toast',              description: '操作中断なしの一時的通知。重大エラーは Modal を使う。',    preview: null,                                  spec: 'components/toast.spec.md',            a11y: '<code>role="status"</code> / <code>"alert"</code>、<code>aria-live</code> の使い分け。' },
